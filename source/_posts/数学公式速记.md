...
若有一个函数$f(x,y)= \begin{cases} xy, & xy \neq 0 \\ x, & y=0\\ y, & x=0 \end{cases}$，那么其函数值一定小于等于各分段表达式绝对值相加，即$f(x,y) \leq |xy| +|x| +|y|$
...
3. 设$a \geq b \geq 0$，则$\begin{cases} 当m>0时,& a^m \geq b^m\\ 当m<0时,& a^m \leq b^m \end{cases}$
...
1. 若$\begin{cases} f(x)在x_0可导\\ f(x_0)=0\\ f'(x_0) \neq 0 (可测变化率) \end{cases}$，则$|f(x)|$在$x_0$处不可导。
2. 若$\begin{cases} f(x)在x_0可导\\f(x_0) \neq 0 \end{cases}$，则$|f(x)|$在$x_0$处可导。
