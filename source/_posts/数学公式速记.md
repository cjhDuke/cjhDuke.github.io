...
设$\rho$为水的密度，$g$为重力加速度，则将某容器中的水全部抽出所做的功为：$W=\rho g \int_a^b x A(x) dx$

## 古鲁金第一定理

如果一条平面曲线 $L$ 绕同一平面内的一条轴旋转一周，并且旋转轴不与曲线相交，那么生成的旋转曲面的面积等于：
$$
A=2 \pi l \cdot r(\bar{x},\bar{y})
$$
其中$r(\bar{x},\bar{y})$为形心到旋转轴的距离
$$
r(\bar{x},\bar{y})=\frac{\int_a^b \frac{|ax+by+c|}{\sqrt{a^2 + b^2}  } \sqrt{1+[f'(x)]^2}dx}{\int_a^b \sqrt{1+[f'(x)]^2} dx}
$$
...
若函数$z=f(x,y)$在点$(x,y)$处可微，则该函数在该点处的偏导数必存在
...
设$z=f(x,y)$在点$(x_0,y_0)$处，$\begin{cases} 一阶偏导数存在 \cr 取极值 \end{cases}$则$f'_x ( x_0, y_0)=0 ,  f'_y(x_0,y_0)=0$

注：偏导数不存在的点也可能是极值点

### 二元函数取极值的充分条件

设$f(x,y)$在点$(x_0,y_0)$的某邻域内连续且有一阶及二阶连续偏导数，又$f'_x(x_0,y_0)=0,f'_y(x_0,y_0)=0$，记

$$
f''\_{xx}(x\_0,y\_0)=A,\quad f''\_{xy}(x\_0,y\_0)=B,\quad f''\_{yy}(x\_0,y\_0)=C
$$

$$
\delta=AC-B^2,\qquad
\begin{cases}
\delta>0,\ A<0 &\Rightarrow \text{极大值}, \cr
\delta>0,\ A>0 &\Rightarrow \text{极小值}, \cr
\delta<0 &\Rightarrow \text{非极值}, \cr
\delta=0 &\Rightarrow \text{方法失效}.
\end{cases}
$$

## 条件最值（拉格朗日乘数法）

求目标函数$u=f(x,y,z)$在下列约束条件下的最值：
...
2. 令
   $$
   \begin{cases}
   F'\_x=f'\_x+\lambda \phi'\_x+\mu \psi'\_x=0, \cr
   F'\_y=f'\_y+\lambda \phi'\_y+\mu \psi'\_y=0, \cr
   F'\_z=f'\_z+\lambda \phi'\_z+\mu \psi'\_z=0, \cr
   F'\_{\lambda}=\phi(x,y,z)=0, \cr
   F'\_{\mu}=\psi(x,y,z)=0.
   \end{cases}
   $$

3. 解上述方程组的备选点$P_i,i=1,2,3,\cdots , n$，并求$f(P_i)$，取其最大值为$u_{max}$，最小值为$u_{min}$
4. 根据实际问题，必存在最值，所得即为所求

## 轮换对称性

在直角坐标系下，若把$x$与$y$对调后，区域$D$不变，则：
$$
\iint \limits_Df(x,y)dxdy= \iint \limits_D f(y,x) dy dx
$$
这就是轮换对称性，一般对于单独的$f(x,y)$或者$f(y,x)$较难计算，但是$f(x,y)+f(y,x)$较为简单的可采用轮换对称性

## 没有初等函数形式的原函数的式子

以下这些函数没有初等函数形式的原函数，一般需要先交换积分次序再进行积分
$$
\frac{\sin x}{x},\quad \frac{\cos x}{x},\quad \frac{\ln{(1+x)}}{x},\quad \frac{1}{\ln x},\quad \sin^2 x,\quad \cos^2 x,\quad \sin \frac{1}{x},\quad \cos \frac{1}{x},\quad \frac{\tan x}{x},\quad \frac{e^x}{x},\quad e^{ax^2+bx+c}(a\neq 0)
$$
...
形如$\frac{dy}{dx}=\phi(\frac{y}{x})$的方程叫作齐次型微分方程。其解法为，令$u=\frac{y}{x}$，则$y=ux \Rightarrow \frac{dy}{dx}=u+x\frac{du}{dx}$，于是原方程变为$x\frac{du}{dx}+u=\phi (u)$，即$\frac{du}{\phi(u)-u}=\frac{dx}{x}$
...
1. 令$y'=p,y''=p'$，则原方程变为一阶方程$\frac{dp}{dx}=f(x,p)$
2. 若求得其通解为$p=\phi(x,C_1)$，即$y'=\phi(x,C_1)$，则原方程得通解为$y=\int \phi(x,C_1)dx+C_2$

### $y''=f(y,y')$型（方程中不显含自变量x）

1. 令$y'=p,y''=\frac{dp}{dx}=\frac{dp}{dy} \cdot \frac{dy}{dx}=\frac{dp}{dy}\cdot p$，则原方程变为一阶方程$p\frac{dp}{dy}=f(y,p)$
2. 若求得其通解为$p=\phi(y,C_1)$，则由$p=\frac{dy}{dx}$可得$\frac{dy}{dx}=\phi(y,C_1)$，分离变量得$\frac{dy}{\phi (y,C_1)}=dx$
3. 两边积分得$\int \frac{dy}{\phi(y,C_1)}=x+C_2$，即可求得原方程的通解

### $y''=f(y')$型

此类型不显含$y$，又不显含$x$，可以参考第一种做法

## 全微分方程

若函数$P(x,y),Q(x,y)$在单连通区域$D$上具有一阶连续偏导数，且在$D$内满足$\frac{\partial Q}{\partial x}=\frac{\partial P}{\partial y}$，则$Pdx+Qdy$是某二元函数$u(x,y)$的全微分，若一阶微分方程写成$P(x,y)dx+Q(x,y)dy=0$的形式时，等式左端表达式是$u(x,y)$的全微分，则称该式为全微分方程

## 二阶常系数齐次线性微分方程

对于$y''+py'+qy=0$，其对应的特征方程为$r^2+pr+q=0$，其对应的通解形式如下
...
方程 $y''+py'+qy=f(x)\ (f(x)\neq0)$ 称为二阶常系数非齐次线性微分方程，其中 $p,q$ 为常数，$f(x)$ 为已知的连续函数，叫作自由项。

### 解的结构

1. 若 $y\_1^{\ast}(x)$ 是 $y''+py'+qy=f\_1(x)$ 的解，$y\_2^{\ast}(x)$ 是 $y''+py'+qy=f\_2(x)$ 的解，则 $y\_1^{\ast}(x)+y\_2^{\ast}(x)$ 是$y''+py'+qy=f\_1(x)+f\_2(x)$ 的解。

2. 设 $y\_1^{\ast},y\_2^{\ast}$ 都是 $y''+py'+qy=f(x)$ 的特解，则 $y\_1^{\ast}-y\_2^{\ast}$ 是对应齐次方程的解。

### 特解的设定

对于 $y''+py'+qy=f(x)$，设 $P_n(x),P_m(x)$ 分别为 $x$ 的 $n$ 次、$m$ 次多项式。
...
\begin{cases}
e^{\alpha x}\text{ 照抄},\cr
Q_n(x)\text{ 为 }x\text{ 的 }n\text{ 次多项式},\cr
k=
\begin{cases}
0,& \alpha\text{ 不是特征根},\cr
1,& \alpha\text{ 是单特征根},\cr
2,& \alpha\text{ 是二重特征根}.
\end{cases}
\end{cases}
$$

2. 当自由项 $f(x)=e^{\alpha x}[P_m(x)\cos\beta x+P_n(x)\sin\beta x]$ 时，特解要设为$y^*=e^{\alpha x}[Q_l^{(1)}(x)\cos\beta x+Q_l^{(2)}(x)\sin\beta x]x^k$其中：$l=\max\{m,n\}$，$Q_l^{(1)}(x)$、$Q_l^{(2)}(x)$ 分别为 $x$ 的两个不同的 $l$ 次多项式，

   $k=\begin{cases}
   0, & \alpha+\beta i\text{ 不是特征根},\cr
   1, & \alpha+\beta i\text{ 是特征根}.
   \end{cases}$

### 通解

若 $y(x)=C\_1y\_1(x)+C\_2y\_2(x)$ 是 $y''+py'+qy=0$ 的通解，$y^{\ast}(x)$ 是$y''+py'+qy=f(x)$的一个特解，则 $y(x)+y^{\ast}(x)$ 是 $y''+py'+qy=f(x)$ 的通解。

##  $n(n\geq 2)$ 阶常系数齐次线性微分方程

1. 若 $r$ 为单实根，写 $Ce^{rx}$；

2. 若 $r$ 为 $k$ 重实根，写$(C_1+C_2x+C_3x^2+\cdots+C_kx^{k-1})e^{rx}$；

3. 若 $r$ 为单复根 $\alpha\pm\beta i$，写$e^{\alpha x}(C_1\cos\beta x+C_2\sin\beta x)$；

4. 若 $r$ 为二重复根 $\alpha\pm\beta i$，写$e^{\alpha x}(C_1\cos\beta x+C_2\sin\beta x+C_3x\cos\beta x+C_4x\sin\beta x)$。

## 能写成：$y''+py'+qy=f(x)$ 形式的微分方程（欧拉方程）

形如$x^2\frac{d^2y}{dx^2}+px\frac{dy}{dx}+qy=f(x)$的方程称为欧拉方程，其中 $p$ 与 $q$ 为常数，$f(x)$ 为已知的连续函数。

欧拉方程有固定的解法：

1. 当 $x>0$ 时，令 $x=e^t$，则 $t=\ln x$，$\frac{dt}{dx}=\frac{1}{x}$，于是$\frac{dy}{dx}=\frac{dy}{dt}\cdot\frac{dt}{dx}=\frac{1}{x}\frac{dy}{dt}$，$\frac{d^2y}{dx^2}=\frac{d}{dx}\left(\frac{1}{x}\frac{dy}{dt}\right)=-\frac{1}{x^2}\frac{dy}{dt}+\frac{1}{x^2}\frac{d^2y}{dt^2}$。方程化为$\frac{d^2y}{dt^2}+(p-1)\frac{dy}{dt}+qy=f(e^t)$，即可求解（最后结果别忘了用 $t=\ln x$ 回代成 $x$ 的函数）。
